...
from hash_util import hashed_block_def
...
class BlockCHAIN:
    def __init__(self, hosting_node_id):
        #Our Starting blokc for the blockchainfrom functools import reduce
import hashlib as aliasHash
import json
import pickle
        ...
        try:
            with open('\\OneDrive\\LiveWKSP\\Python\\MaxSPracticalGuide\\filesCreatedFromPython\\blockChain.txt', mode='r') as fileVariable:
                ...
                for block in blockchain:
                    convertedTXVar = [Transaction(tx['sender'], tx['recipient'],tx['amount']) for tx in block['transactions']]                
...
# The optional one is optional because it has a default value => [1]
   ...
   # blockchain.append([last_transaction, transaction_amount])
    ...
    def load_data(self):
        ...
        try:
            with open('*****************************************blockChain.txt', mode='r') as fileVariable:
                file_content =  fileVariable.readlines()
                #file_content = pickle.loads(fileVariable.read()) 
                # blockchain = file_content['chain']
                # open_transactions = file_content['ot']
